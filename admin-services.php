...
/*
  ...
  ALTER TABLE services
    ADD COLUMN media_json TEXT NULL AFTER image_path;
*/
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    $stmt = $pdo->prepare("SELECT image_path, media_json FROM services WHERE id = :id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST['delete_id'])) {
    ...
    // Existing media JSON string from form (for edit)
    ...
    // Flags for deleting existing items (array of indexes)
    ...
    if (!empty($_FILES['image']['name']) && is_array($_FILES['image']['name'])) {
        foreach ($_FILES['image']['name'] as $idx => $name) {
            ...
            if ($_FILES['image']['error'][$idx] === UPLOAD_ERR_OK) {
                ...
                $safeName  = uniqid('svc_img_', true) . '.' . $ext;
    ...
    if (!empty($_FILES['video']['name']) && is_array($_FILES['video']['name'])) {
        foreach ($_FILES['video']['name'] as $idx => $name) {
            ...
            if ($_FILES['video']['error'][$idx] === UPLOAD_ERR_OK) {
                $ext       = strtolower(pathinfo($name, PATHINFO_EXTENSION));
                $safeName  = uniqid('svc_vid_', true) . '.' . $ext;
                $targetAbs = $uploadDirAbs . $safeName;
...
// Prepare data for JS (for Edit button)
$servicesForJs = [];
foreach ($services as $s) {
    // Build media preview for admin: use media_json if available else image_path
    $mediaItems = [];
    if (!empty($s['media_json'])) {
        $decoded = json_decode($s['media_json'], true);
        if (is_array($decoded)) {
            $mediaItems = $decoded;
        }
    } elseif (!empty($s['image_path'])) {
        $parts = array_filter(array_map('trim', explode(',', $s['image_path'])));
        foreach ($parts as $p) {
            $mediaItems[] = ['type' => 'image', 'src' => $p];
        }
    }

    $servicesForJs[(int)$s['id']] = [
        'id'             => (int)$s['id'],
        'slug'           => $s['slug'],
        'en_title'       => $s['en_title'],
        'en_description' => $s['en_description'],
...
<head>
  <meta charset="UTF-8">
  <title>Admin Services | Muadh Al Zadjali</title>
...
<main>
  ...
  <div class="section-card">
    <h2 style="font-size:16px;margin-bottom:8px;" id="form-title">Add New Service</h2>
    <p style="font-size:12px;margin-bottom:6px;" id="form-helper">
      Enter the service details in English. You can upload multiple images and videos; they will auto‑slide on the Services page.
    </p>
    ...
    <form method="post" enctype="multipart/form-data" id="serviceForm">
      ...
      <label for="slug">Slug (unique key, e.g. residential)</label>
  ...
  <div class="section-card">
    <h2 style="font-size:16px;margin-bottom:8px;">Existing Services</h2>
    <table>
      <thead>
        <tr>
          <th>ID</th>
      ...
      <tbody>
      <?php if (!$services): ?>
        <tr><td colspan="6">No services added yet.</td></tr>
      <?php else: ?>
        <?php foreach ($services as $s):
          $mediaPreview = [];
          if (!empty($s['media_json'])) {
              $mj = json_decode($s['media_json'], true);
        ...
        ?>
          <tr>
            ...
            <td><?php echo htmlspecialchars($s['en_title']); ?></td>
            <td>
            ...
            <td class="actions-cell">
              <button
                type="button"
                class="btn-small"
                onclick="startEdit(<?php echo (int)$s['id']; ?>)"
              >Edit</button>
...
<script>
  const servicesData   = <?php echo json_encode($servicesForJs, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT); ?>;
  ...
  function resetFormToAdd() {
    ...
    formHelper.textContent = 'Enter the service details in English. You can upload multiple images and videos; they will auto‑slide on the Services page.';
  ...
  function renderExistingMediaList(media) {
    ...
    media.forEach((item, idx) => {
      ...
      // hidden field that actually marks delete when unchecked
      checkbox.addEventListener('change', () => {
        // we only send delete_media[] for unchecked items
  ...
  function startEdit(id) {
    ...
    sortInput.value        = String(s.sort_order);
    titleInput.value       = s.en_title;
    descInput.value        = s.en_description;
    fileImageInput.value   = '';
    fileVideoInput.value   = '';

    // existing media for this service
    const media = s.media || [];
    existingMediaJsonInput.value = JSON.stringify(media);
    renderExistingMediaList(media);

    formTitle.textContent  = 'Edit Service #' + s.id;
